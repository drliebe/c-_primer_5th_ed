...
{
	size_t ret = 0;	
	auto l = lhs.check(lhs.curr, "LHS error");
	auto r = rhs.check(rhs.curr, "RHS error");
	if (*l == *r)
		ret = lhs.curr - rhs.curr;
	else
		throw std::runtime_error("invalid pointer comparison");
	return ret;
}

std::string &StrBlobPtr::operator*()
{
	auto p = check(curr, "Error dereferencing");
	return (*p)[curr];
}
...
{
public:
	PtoStrBlobPtr() : ptr(nullptr) { }
	PtoStrBlobPtr(StrBlobPtr *p) : ptr(p) { }
	StrBlobPtr &operator*();
	StrBlobPtr *operator->();
private:
	StrBlobPtr *ptr;
...
StrBlobPtr &PtoStrBlobPtr::operator*()
{
	return *ptr;
}
...
{
	auto p = check(curr, "dereference off vector");
	return (*p)[curr];
}
